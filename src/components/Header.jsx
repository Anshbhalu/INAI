...
const Dropdown = ({ label, to, links, isOpen, onToggle, dropdownKey }) => (
  <li
    ...
    onClick={() => window.matchMedia('(max-width: 900px)').matches && onToggle(isOpen ? null : dropdownKey)}
  >
    <span
    ...
    <ul className="dropdown-menu">
      {links.map((link) => (
        <li key={link.to}>
          <NavLink to={link.to} className="dropdown-link" onClick={() => onToggle(null)}>
            {link.label}
          </NavLink>
        </li>
      ))}
...
const Header = () => {
  ...
  const portfolioLinks = [
    { to: '/inai_life', label: 'INAI_LIFE' },
    { to: '/influwket', label: 'INFLUWKET' },
    { to: '/offiket', label: 'OFFIKET' },
